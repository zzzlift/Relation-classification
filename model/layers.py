...
class PositionAwareAttention(nn.Module):
    ...
    def __init__(self, input_size, query_size, feature_size, attn_size):
        super(PositionAwareAttention, self).__init__()
        pedim=0
        self.input_size = input_size+pedim
        self.query_size = query_size
        self.feature_size = query_size
        self.attn_size = attn_size
        self.ulinear = nn.Linear(input_size+pedim, attn_size)
        self.vlinear = nn.Linear(query_size, attn_size, bias=False)
        if query_size > 0:
            self.wlinear = nn.Linear(query_size, attn_size, bias=False)
        else:
            self.wlinear = None
        self.tlinear = nn.Linear(attn_size, 1)
        self.init_weights()
    ...
    def forward(self, x, x_mask, q, f):
        ...
        x_proj = self.ulinear(x.contiguous().view(-1, self.input_size)).view(
            batch_size, seq_len, self.attn_size)
        # x_mean=torch.mean(xf,dim=1)
        x_mean=torch.sum(x,dim=1)/((1-x_mask).sum(-1,keepdim=True).float()+1e-12)
        q_proj = self.vlinear(x_mean).contiguous().view(
            batch_size, self.attn_size).unsqueeze(1).expand(
                batch_size, seq_len, self.attn_size)
        # print(x_proj.size(),q_proj.size())
        if self.wlinear is not None:
            f_proj = self.wlinear(x.contiguous().view(-1, self.feature_size)).view(
                batch_size, seq_len, self.attn_size)
            # projs = [x_proj, q_proj, f_proj]
        else:
            # projs = [x_proj, q_proj]
        ...
        # scores = self.tlinear(torch.tanh(sum(projs)).view(-1, self.attn_size)).view(
        #     batch_size, seq_len)
        # # (b,s,1) (b,s,d) = (b,s,d) => (b,s)
        scores=torch.sum(x_proj*q_proj,dim=-1)
        # scores=x_proj.bmm(q_proj.permute(0,2,1))
        # print(scores.size())
        ...
        weights = F.softmax(scores, dim=1)
        # weighted average input vectors
        outputs = weights.unsqueeze(1).bmm(f_proj).squeeze(1)
        return outputs
